...
3. AI 可以提出摘要、标签、分类和关系建议，但正式知识必须由人确认。
...
- **同步与确认解耦**：来源原文按授权规则自动同步，不要求逐页批准；必须人工确认的是 AI 摘要、
  标签、分类、关系以及动态收件箱中的正式入库决定。
...
| --- | --- | --- | --- |
| 家庭成员 OneNote | 每位成员分别绑定自己的 Microsoft 账户；单向、增量同步 | 外部原文完整接入 | 是；先用一位成员的一个笔记本验证 |
| 现有投资笔记/随手记 | 使用内部引用和搜索投影，不复制出第二份可编辑正文 | 原模型仍为权威数据 | 是 |
...
| 关注人物与机构动态 | 通过明确登记的官方/授权来源定时获取，进入动态收件箱 | 候选内容，人工筛选后入库 | 后续动态追踪阶段 |
| 在线图书 | 原始书籍文件、书目、版本、阅读位置、划线和批注 | 阅读模块拥有阅读事实 | 后续在线阅读模块 |
...
两者不能合并成一个状态字段。例如同步成功但 AI 暂时不可用时，用户仍应能够浏览原文。
...
- 定时获取的新内容先进入“动态收件箱”，用户可以保留、忽略、确认分类后再进入正式知识库。
...
- 知识中心通过内部引用与搜索投影展示，用户从结果页编辑时跳转或调用原模块的正式编辑流程。
...
第一版搜索标题、规范正文、人工确认摘要、标签、作者/人物、来源名称和现有随手记。搜索结果
显示来源、可见范围、内容时间、整理状态和命中片段。
...
- **首页工作台**：以“今天需要做什么”为主线，显示今日新增、AI 待确认、同步异常、继续阅读、
  最近确认和关注人物重要更新；上传与导入能力未落地前必须明确标记；
- **AI 人工确认**：原文与 AI 建议左右并排，逐项接受、修改或拒绝；整篇确认保留预览步骤，
  最后一项处理后自动进入下一篇；重复和忽略操作在状态与审计模型完成后开放；
- **知识列表**：作为全部可见知识的统一目录，不再拆成“我的知识”和“家庭知识”两个页面；
  默认显示当前登录成员的内容，通过成员筛选切换其他成员或全部，并按类型、来源、人物、标签、
  日期和状态筛选；
- **动态收件箱**：自动进入的新内容和人物/机构动态在此筛选，不未经确认直接污染长期知识；
- **关注人物**：人物主页与时间线，必须区分本人发言、机构发布、采访原文、媒体转述和 AI 提炼；
  正式人物身份与连接器后续开放；
- **在线书库**：图书、阅读计划和继续阅读，由 `reading` 独立负责，后续阶段开放；
- **主题与标签**：按标签、主题和后续关系浏览；
- **来源与同步**：账户绑定、笔记本选择、同步进度、失败明细和重试；
- **统一搜索**：位于知识列表，跨已接入来源搜索，并能按来源、成员、时间和类型筛选。

页面关系固定为：首页负责行动提示，动态收件箱负责新内容流转，AI 人工确认负责核对建议，关注
人物负责人物视角，知识列表负责统一目录和筛选；这些视图引用同一知识条目与统一详情，不复制
业务数据。

页面设计借鉴成熟产品的长处，但按家庭工作台重新设计：

- 借鉴 Notion 的清晰导航和属性展示；
- 借鉴 Readwise Reader 的“收件箱—整理—资料库”流转；
...
- [ ] **AC-KB-019** 普通成员可以在知识中心按个人/家庭、来源、成员、时间、标签和整理状态
  浏览；详情页显示原文、来源、版本、附件、可见范围和确认状态。
- [ ] **AC-KB-020** 搜索覆盖 OneNote 和随手记的标题、正文、已确认摘要、标签、作者/人物和
  来源名称，结果提供命中片段和原文入口。
