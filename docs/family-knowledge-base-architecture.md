...
3. AI 可以提出摘要、标签、分类和关系建议，但不能决定内容是否正式归档或进入精选知识；成员的
   分区规则、明确勾选、批次确认或专业模块中的归档操作构成人工选择。
...
- **人对精选知识负责**：AI 结果默认是待确认建议，不能未经确认直接改变正式分类或进入精选知识。
- **同步、归档去向与整理状态分开**：同步回答“是否取得原文”，来源规则回答“归档、归档并加入
  待整理还是仅同步”，整理状态回答“是否准备整理、建议是否生成和是否已确认”。三者不得合并。
...
| --- | --- | --- | --- |
| 家庭成员 OneNote | 每位成员分别绑定 Microsoft 账户；按分区归档、归档并待整理或仅同步 | 外部原文完整接入 | 是；先用一位成员的一个笔记本验证 |
| 现有投资笔记/随手记 | 成员勾选后建立归档引用、搜索投影并加入待整理 | 原模型仍为权威数据 | 是；默认不进入知识库 |
...
| 关注人物与机构动态 | 独立多来源订阅、AI 摘要、关键词和事件聚合；成员按需归档 | 人物模块拥有资讯流 | 后续动态追踪阶段 |
| 在线图书 | 阅读模块管理图书与阅读事实；获选笔记、划线、总结和思维导图按需归档 | 阅读模块拥有阅读事实 | 后续在线阅读模块 |
...
精选整理状态回答“资料是否准备进入精选知识，以及整理进行到哪里”：

- 未选择、待整理、待 AI 处理、待人工确认、已确认精选、已忽略。

“归档资料”是全部正式资料的全集，“待整理”和“精选知识”是其中两个整理状态视图；资料进入
待整理或精选后仍保留在归档资料中。三类状态不能合并成一个字段。例如已归档文章可以从未生成
AI 摘要；生成新建议也不能让它从归档资料消失。页面面向成员主要显示“保存状态、整理状态、
AI 进度”。

来源级云端 AI 授权只回答“这个来源以后被成员主动选择的正文是否可以发送给云端”，不代表系统
可以自动处理整个来源。AI 任务必须保存成员明确选择的文档 ID；未选中的待整理资料不能因为同属
一个已授权来源而被处理。成员也可以只授权本次选中文档，任务结束后不改变来源级授权。
...
- OneNote 笔记本、分区和页面层级作为来源位置独立保存和展示，不自动成为内容分类。人工或 AI
  确认分类后，后续同步不得覆盖；页面移动只更新原始位置，不静默改变人工整理结果。
- 首次全量后采用增量同步，并定期进行完整对账，以识别游标遗漏、来源删除或权限变化。
- 用户内容同步成功后立即可浏览；AI 处理失败不得阻塞同步结果。
- 每个已发现分区可配置“进入归档资料、归档并加入待整理、仅同步”，并有新分区默认规则；
  心得和经验可以先归档，准备提炼的摘录可以归档并加入待整理。规则默认只影响以后首次同步的新页面，
  成员明确选择后才批量应用于已有内容。
...
- 导入确认后默认直接进入归档资料，不自动进入待整理或精选知识；同一文档后续升级为精选知识时
  仍保留在归档资料中。
- Markdown 财经日记和 HTML 公众号历史文章按作者归入“关注人物 → 对应人物 → 历史文章”；
  技术格式、导入批次和文件路径保留为来源属性，不单独占用资料分区。
...
- 成员对高价值长文、完整访谈或重要观点点击“归档”后，才把获选原文、人物身份、来源等级和
  原始链接固化到知识底座；可只归档，也可归档并加入待整理，整理确认后进入精选知识。
...
#### 6.3.1 人物文章生成的 AI 专题成果

- 由一组历史文章生成的知识手册、思维导图或知识体系图属于 `KnowledgeArtifact` 派生成果，
  不复制成普通原文，也不替代参与生成的历史文章。
- 每次上传创建不可变 `KnowledgeArtifactVersion`，保存 AI 原文件、SHA-256、生成工具、模型、
  提示词版本、来源规模、截止日期和确认状态；重新生成只增加版本，不无痕覆盖。
- 出处优先按“规范文章标题＋发布日期”映射到 `KnowledgeDocument` 的精确
  `KnowledgeRevision`；唯一标题可以作为降级匹配。重复候选和未匹配项必须保留清单，不能猜测。
- 专题成果默认显示“AI 生成 · 待人工确认”，确认人和确认时间单独记录。点击已映射出处时展示
  当时绑定的固定原文版本；当前原文更新不会让旧引用静默跳到新含义。
- 自包含交互 HTML 只能在受鉴权的沙箱 iframe 中运行：不授予同源身份，不允许联网、表单、
  弹窗或顶层跳转，使用独立 CSP；原始 HTML 仍只允许下载，不在主页面直接 `safe` 渲染。
- 少量成果优先显示在对应人物主页；跨人物成果达到稳定规模后再考虑独立一级入口。统一搜索将
  整个成果作为一条“AI 专题成果”投影，不把数百个 AI 知识点伪装成数百篇人工确认的知识。

### 6.4 现有投资笔记/随手记

- 现有记录继续以原模型为唯一可编辑事实，不迁移成重复正文。
- 随手记默认不进入知识中心，也不进入待整理。成员在新建或编辑时明确勾选“加入家庭知识库”后，
  才创建内部引用与搜索投影，并同时进入归档资料和待整理；整理后由成员确认进入精选知识。取消
  勾选只删除可重建投影，不删除原笔记。
...
- 书籍正文、全部划线和阅读行为不自动进入知识库；成员选择的笔记、划线、AI 重点总结、思维导图
  和获准共享的讨论组成可追溯成果包，按权限主动归档或归档并加入待整理；
...
- 首次发送提供“仅本次所选正文”和“今后允许该来源”两种授权；持续来源授权只减少后续确认，
  不自动创建任务。家庭管理员不能代替其他成员作出一次性或持续正文外发授权。
...
成员明确加入知识库的随手记。资料目录中的搜索默认覆盖全部归档资料；仅同步内容只在来源管理
或高级检查入口查看，不混入日常资料目录。搜索结果显示来源、原始位置、可见范围、内容时间、
整理状态和命中片段。
...
主导航保留“知识中心”；OneNote、随手记、导入批次等普通来源不各自增加顶级导航。高频资讯的
“AI 情报”作为独立入口保留，使用适合动态扫描的页面框架；它与知识中心共享身份和归档桥梁，
但不强制共用页面导航。知识中心内部建议包含：

- **首页工作台**：以“今天需要做什么”为主线，显示今日入库、待整理、AI 建议、同步异常、
  继续阅读和最近知识；上传与导入能力未落地前必须明确标记；
- **等待确认**：作为待整理的子流程展示原文与 AI 建议，逐项接受、修改或拒绝；整篇确认保留
  预览步骤，最后一项处理后自动进入下一篇；不在一级导航中另设“AI 建议确认”；
- **资料分区**：固定为精选知识、待整理、归档资料。归档资料是正式资料全集；待整理只放已经
  准备进入精选知识但尚未完成提炼的资料；精选知识只放整理完成并经成员确认的内容。三种视图
  默认当前成员，可通过成员、类型、来源、人物、标签、日期和状态筛选；桌面端二级目录可在
  “按分类”和“按来源”之间切换；
- **二级目录**：按分类浏览内容主题；按来源固定显示到第二级，如 OneNote→笔记本、随手记→作者、
  关注人物→人物。来源目录与人工分类互不替代；
- **待整理进度**：只提供“全部待整理、尚未整理、等待确认”三个成员筛选。AI 排队/运行中只在
  资料上显示即时状态；单篇和批量 AI 整理都从这里发起，只处理成员明确选中的资料，完成后进入
  “等待确认”；收件箱、规范化等内部状态保留给任务编排和审计，不作为家庭成员筛选；
- **知识中心 → 关注人物**：展示人物的历史文章、已归档情报和长期知识；通过稳定人物映射连接
  AI 情报的人物主页，不在知识中心复制最新资讯流；
- **AI 情报**：独立承担多来源聚合订阅、人物主页和动态时间线，必须区分本人发言、机构发布、
  采访原文、媒体转述和 AI 摘要；普通资讯不进入归档资料或待整理，成员主动执行“保存为知识”
  或“保存并加入待整理”时，才固化当时的事件与证据快照并建立双向链接；
- **在线书库**：图书、阅读计划和继续阅读，由 `reading` 独立负责，后续阶段开放；
- **主题与标签**：按标签、主题和后续关系浏览；
- **来源与同步**：账户绑定、来源身份、笔记本选择、同步规则、云端 AI 持续授权、同步进度、失败
  明细和重试；来源页可只读链接到筛选后的待整理资料，但不直接发起 AI 内容整理；
- **统一搜索**：位于资料目录，默认跨全部归档资料搜索，并能按来源、成员、时间和类型筛选。

页面关系固定为：首页负责行动提示，归档资料负责正式资料全集和检索，待整理负责需要提炼的资料，
其中“等待确认”负责核对 AI 建议，精选知识负责长期复用结果，关注人物和在线阅读负责各自领域流程；
专业模块只在成员明确选择时归档成果或建立投影，不复制业务事实。
...
- 同步任务、状态、重试、审计和来源断开；
- AI 摘要、标签和分类建议及人工确认；
- 随手记明确勾选后的归档、待整理、统一浏览与搜索投影；
- OneNote 分区级归档、归档并待整理和仅同步；
- 统一浏览、筛选和全文搜索；
...
- 财经大 V 微信公众号 HTML 导入；
- 导入试算、重复识别、错误报告和批次回滚；
- 主动导入资料统一进入归档资料，并按作者显示在“关注人物 → 对应人物 → 历史文章”；不自动进入
  待整理或精选知识，成员选择、整理并确认后才进入精选知识；
- 在大体量真实语料上调整搜索、分类和人物/主题组织。

第二版已经确认的批次模型、预览/确认/回滚流程、HTML 安全边界和真实样本试导入记录见
[《Markdown / HTML 历史资料批量导入》](knowledge-import-v2-design.md)。该文档只记录第二版的
可变实现与验证进度，不改变本架构确定的模块边界。
...
- 成员对重要长文或观点执行“归档”或“归档并加入待整理”。

### 第四版：在线阅读

- EPUB/PDF 书架、阅读器、年度计划、进度；
- 划线、批注、共享评论及权限；
- 成员选择的划线、笔记、总结和思维导图作为成果包按需归档；
- AI 总结和思维导图另行验收。
...
- [ ] **AC-KB-011** 现有投资笔记在知识中心可以浏览和搜索，标题、正文、分类、标签、成员和
  可见性与原记录一致；勾选加入家庭知识库后同时进入归档资料和待整理。
...
- [ ] **AC-KB-018** 批量确认前显示影响数量和变更预览；任务部分失败时可定位到具体项目并
  重试，不把失败项标成已确认。
- [ ] **AC-KB-039** 归档资料始终包含全部正式资料；资料加入待整理或确认为精选知识后不会从
  归档资料消失。未进入待整理的归档资料不会被 AI 任务自动整理或自动提升为精选知识；来源持续
  授权后，AI 批量任务仍只处理成员在待整理中明确选中的资料。
...
- [ ] **AC-KB-019** 普通成员可以在知识中心按个人/家庭、来源、来源分区、分类、成员、时间、
  ...
  和确认状态。
- [ ] **AC-KB-020** 搜索覆盖全部归档资料中 OneNote、随手记、Markdown 和 HTML 的标题、正文、
  已确认摘要、分类、标签、作者/人物、来源名称及来源分区路径，结果提供命中片段和原文入口。
