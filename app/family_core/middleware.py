...
VIEWER_AI_WRITE_PATH = re.compile(
    r"^/ai/(?:conversations/(?:new|\d+/(?:archive|rename|delete|ask))/|requests/\d+/(?:cancel|recover)/|outbound-authorization/|memories/(?:new|\d+/(?:confirm|revise|delete))/|messages/\d+/share-preview/|shares/\d+/(?:publish|refresh|withdraw)/)$"
)
...
class ActiveHouseholdMemberMiddleware:
    ...
    def __call__(self, request):
        ...
        if member is None or not member.is_active:
            ...
            if member.role == FamilyMember.ROLE_VIEWER and request.method not in SAFE_METHODS and not viewer_personal_ai_action:
